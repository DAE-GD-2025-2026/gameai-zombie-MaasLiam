...
UStudentPerceptorMaasLiam::UStudentPerceptorMaasLiam()
{
	PrimaryComponentTick.bCanEverTick = true;
}

void UStudentPerceptorMaasLiam::BeginPlay()
{
	Super::BeginPlay();

	if (auto PerceptionComp = GetOwner()->GetComponentByClass<UAIPerceptionComponent>())
	{
		PerceptionComp->OnTargetPerceptionUpdated.AddDynamic(this, &UStudentPerceptorMaasLiam::OnPerceptionUpdated);
	}
}

void UStudentPerceptorMaasLiam::TickComponent(
	float DeltaTime,
	ELevelTick TickType,
...
void UStudentPerceptorMaasLiam::OnPerceptionUpdated(AActor* Actor, FAIStimulus Stimulus)
{
	if (!Actor) return;
...
bool UStudentPerceptorMaasLiam::IsZombieActor(AActor* Actor) const
{
...
void UStudentPerceptorMaasLiam::RememberZombie(AActor* Zombie)
{
	if (!Zombie) return;
...
void UStudentPerceptorMaasLiam::UpdateZombieMemory(float DeltaTime)
{
