...
bool FZombieCombatHelperMaasLiam::TryUseWeapon(UActorComponent* InventoryComponent)
{
...
bool FZombieCombatHelperMaasLiam::DoesInventorySlotContainItemType(UActorComponent* InventoryComponent, int32 SlotIndex, const FString& ItemType)
{
	if (!InventoryComponent)
	{
		return false;
...
bool FZombieCombatHelperMaasLiam::TryUseItemInSlot(UActorComponent* InventoryComponent, int32 SlotIndex)
{
	if (!InventoryComponent)
	{
		return false;
...
bool FZombieCombatHelperMaasLiam::TryRemoveItemInSlot(UActorComponent* InventoryComponent, int32 SlotIndex)
{
	if (!InventoryComponent)
	{
		return false;
...
int32 FZombieCombatHelperMaasLiam::GetInventoryCapacity(UActorComponent* InventoryComponent)
{
	if (!InventoryComponent)
	{
		return 5;
...
int32 FZombieCombatHelperMaasLiam::GetInventorySlotItemValue(UActorComponent* InventoryComponent, int32 SlotIndex)
{
	if (!InventoryComponent)
	{
		return 0;
